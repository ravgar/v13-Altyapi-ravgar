...
module.exports = { name: "vmute", aliases: ["voicemute"],  desc: "Taslak Komut",
...
if(!message.member.permissions.has("8") && !Server.VoiceAuth.some(rol => message.member.roles.cache.has(rol)) && !Server.ÜstYönetimRoller.some(rol => message.member.roles.cache.has(rol)) && !Server.OwnerRolleri.some(rol => message.member.roles.cache.has(rol))) return message.react(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")}`)
...
if (Server.VmuteLimit > 0 && voiceMuteLimit.has(message.author.id) && voiceMuteLimit.get(message.author.id) == Server.VmuteLimit) return message.reply(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")} \`Hatalı İşlem\` Saatlik VoiceMute limitini doldurdun!`), message.react(`${client.emojis.cache.find(x => x.name === "ravgar_carpi")}`)
let MuteThisCheck = await Punitives.findOne({guildID: message.guild.id, victimID: Member.id, Type: "VOICE-MUTE", Active: true})
...
if (Server.VmuteLimit > 0) { if (!voiceMuteLimit.has(message.author.id)) voiceMuteLimit.set(message.author.id, 1);
...
let Penal = await new Punitives({ No: count, guildID: Settings.guildID, execID: message.author.id, victimID: Member.id, Active: true, Reason: seçilenSebep.label, nowDate: Date.now(), finishDate: Date.now()+ms(seçilenSebep.date),   Type: "VOICE-MUTE"}).save()
} else {
